...
if command -v docker-compose &> /dev/null || command -v docker &> /dev/null; then
    ...
    if [ -z "$DATABASE_URL" ]; then
        ...
        read -p "Database name [ekidna_db]: " DB_NAME
        DB_NAME=${DB_NAME:-ekidna_db}
        read -p "Database user [ekidna]: " DB_USER
        DB_USER=${DB_USER:-ekidna}

        psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f migrations/002_add_serial_number.sql
    else
    ...
    echo "  docker-compose exec db psql -U ekidna -d ekidna_db -f /app/migrations/002_add_serial_number.sql"
    echo ""
    echo "Method 2 - Direct psql:"
    echo "  psql -h localhost -U ekidna -d ekidna_db -f migrations/002_add_serial_number.sql"
    echo ""
    exit 1
fi
